...
export default defineComponent({
  setup() {
    ...
    //チェックされたトッピングを格納する配列
    let selectedToppings = ref(new Array<orderTopping>());
    //トッピングにチェックされているかどうか
    ...
    let addOrderTopping = () => {
      if (checkFlag.value === true) {
        selectedToppings.push(
          new orderTopping(0, 0, 0, currentOrderItem.value.orderToppingList)
        );
      }
    };
